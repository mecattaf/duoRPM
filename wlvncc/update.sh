#!/bin/bash
# Script to check for updates to wlvncc and update the spec file accordingly
...
if [ "$LATEST_RELEASE" = "null" ] || [ -z "$LATEST_RELEASE" ]; then
    ...
    if [ "$LATEST_COMMIT" != "$CURRENT_COMMIT" ]; then
        ...
        NEW_VERSION="0.1.0~git${COMMIT_DATE}.${LATEST_COMMIT:0:7}"
        ...
        git add "$SPEC"
        git commit -m "Update wlvncc to snapshot ${LATEST_COMMIT:0:7} [build-gcc]"
        git push
        
        echo "Updated $SPEC to snapshot $NEW_VERSION"
    else
        echo "wlvncc is already at the latest commit: ${CURRENT_COMMIT:0:7}"
    fi
else
    ...
    if [ "$LATEST" != "$CURRENT" ]; then
        ...
        if curl --output /dev/null --silent --head --fail "$SOURCE_URL"; then
            ...
            git add "$SPEC"
            git commit -m "Update wlvncc to release $LATEST [build-gcc]"
            git push
        ...
        echo "wlvncc is already at the latest release: $CURRENT"
    fi
fi
